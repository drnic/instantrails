...
#
# $Id: source_parser.rb,v 1.3 2005/11/15 03:29:41 jonathanm Exp $
#
...
module FreeRIDE
  module Tools
    ...
    class SourceParser
      ...
      # The access time of the file is used to determine if the cached value is up to date
      ...
      class CacheStructure
        ...
        def initialize(fn, path, src)
          ...
          @source = src
          @f = File.join(path,fn)
          @cache_atime = File.atime(@f)
          yield(self) if block_given?
        end
        
        def up2_date?()
          atime = File.atime(@f)
          return @cache_atime == atime
        end
