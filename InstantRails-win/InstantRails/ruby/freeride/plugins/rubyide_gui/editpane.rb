...
#
# $Id: editpane.rb,v 1.16 2005/03/12 21:02:41 ljulliar Exp $
#
  ...
  module GUI
    ...
    class EditPane < Component
      ...
      def close(close_all=false)
        ...
        slot = self.neighbor
        if slot == nil
          @plugin["/system/ui/current/EditPane"].unlink
        else
      ...
      def save_as
        ...
        answer = 'no'
        while answer!='yes'
          answer, filename = @cmd_mgr.command("App/Services/FileSaveAs").invoke(file)
          return answer if answer == 'cancel'
