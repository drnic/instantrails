...
#
# $Id: source_browser.rb,v 1.5 2005/03/30 14:21:15 ljulliar Exp $
#
  ...
  module FoxRenderer
	...
	@busy_updating.synchronize do
          ...
          if editpane
            ...
            @tree.clear_nodes
            append_to_tree(source.top_level_context, @tree.root_node) if source
            expandTree(@tree)
	    @tree.update
          end
	end
      ...
      def append_to_tree(source_tree, node, icon=nil)
        if source_tree.name == ""
        ...
        new_node = @tree.add_child_node(node, name, icon)
        @tree.set_node_data(new_node, source_tree.start_point.line_no)
      
        source_tree.each_singleton_method do |meth|
          n = @tree.add_child_node(new_node, meth.name, @plugin["/system/ui/icons/SourceBrowser/method"].data)  
          @tree.set_node_data(n, meth.start_point.line_no)
        end
        ...
        source_tree.each_module do |mod|
          append_to_tree(mod, new_node, @plugin["/system/ui/icons/SourceBrowser/module"].data)
        end
        
        source_tree.each_singleton_class do |c|
          append_to_tree(c, new_node, @plugin["/system/ui/icons/SourceBrowser/class"].data)
        end
        
        source_tree.each_class do |klass|
          append_to_tree(klass, new_node, @plugin["/system/ui/icons/SourceBrowser/class"].data)
        end
