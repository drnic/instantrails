...
#
# $Id: script_runner.rb,v 1.27 2005/02/19 14:41:28 ljulliar Exp $
#
...
module FreeRIDE
  class ScriptRunner
	...
	@@script_runner.kill if @@script_runner
        @@script_runner = ScriptRunner.new(plugin)
      end
    ...
    def initialize(plugin)
      ...
      if dbg_plugin.properties['save_before_running']
      ...
      if dbg_plugin.properties['run_in_terminal'] && RUBY_PLATFORM =~ /(mswin32|mingw32)/
      ...
      exec_args = dbg_plugin.properties['cmd_line_options']
      exec_dir = dbg_plugin.properties['working_dir']
      ...
      unless find_ruby || FileTest.exist?(ruby_path)
        cmd_mgr.command('App/Services/MessageBox').invoke("Where is Ruby?",
          "I can't find the Ruby interpreter '#{ruby_path}'. Please configure the path to ruby in the Debugger/Run preference box")
	return
      end

      command = "#{ruby_path} -C \"#{exec_dir}\" -r \"#{starter_file}\" \"#{file}\" #{exec_args}"
      if dbg_plugin.properties['run_in_terminal']
	...
	path_sep = ':'
      ...
      ENV['PATH'].split(path_sep).each do |dir|
	ruby_names.each do |f|
