...
#
# $Id: script_runner.rb,v 1.31 2005/12/10 09:49:00 jonathanm Exp $
#
...
module FreeRIDE
  class ScriptRunner
    ...
    # All files opened with the default project use the default (general)
    # debug settings.
    def initialize(plugin)
      @plugin = plugin["/plugins/rubyide_tools_fox_script_runner"].manager
      @cmd_mgr = cmd_mgr = plugin["/system/ui/commands"].manager
      ...
      if plugin["properties/name"] == "Default Project"
        setting_props = dbg_plugin.properties
      else
        setting_props = plugin["properties"]
      end
      ...
      if setting_props['save_before_running'].data
      ...
      #ep_slot['actions/save'].invoke if ep_slot['actions/modified'].invoke
      #unless ep_slot['actions/modified'].invoke
      file = ep_slot.manager.filename
      if setting_props['run_in_terminal'].data && RUBY_PLATFORM =~ /(mswin32|mingw32)/
      ...
      ruby_path = dbg_plugin.properties['path_to_ruby'] # TODO Fixme for projects
      #ruby_path = 'ruby' if (ruby_path == '' || ruby_path == nil)
      exec_args = setting_props['cmd_line_options'].data
      exec_dir = setting_props['working_dir'].data
	...
	return
      ...
      if plugin["properties/name"].data != "Default Project"
        setting_props["source_directories"].data.each do |s| command += " -I \"#{s}\"" end
        setting_props["required_directories"].data.each do |r| command += " -I \"#{r}\"" end
      end
      command += " \"#{file}\" #{exec_args}"
      if setting_props['run_in_terminal'].data
