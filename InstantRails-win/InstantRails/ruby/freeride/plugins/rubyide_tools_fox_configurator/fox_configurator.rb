...
#
# $Id: fox_configurator.rb,v 1.6 2005/02/20 08:04:01 ljulliar Exp $
#
  ...
  module FoxRenderer
    ...
    class Configurator
	  ...
	  @config_tree.setNumVisible(10)
          ...
          FXButton.new(btn_box, "&Cancel", nil, self, FXDialogBox::ID_CANCEL,
                       LAYOUT_RIGHT|FRAME_RAISED|FRAME_THICK, 0, 0, 0, 0, 20, 20)
	  ...
	  ok_btn.connect(SEL_COMMAND) do  |sender, sel, scn|
        ...
        def on_selected(sender,sel,item)
          config_slot = @config_tree.getItemData(item)
          @config_title.text = config_slot.attr_description
          @config_frame.current = config_slot.attr_pane_idx
          config_slot.manager.get_config_properties(config_slot)
        end
