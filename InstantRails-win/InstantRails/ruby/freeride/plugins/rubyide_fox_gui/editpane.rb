...
#
# $Id: editpane.rb,v 1.41 2005/03/20 08:40:20 ljulliar Exp $
#
  ...
  module FoxRenderer
	...
	def onClicked(sender,sel,ptr)
      ...
      class ReplaceDialog < FXDialogBox
        ...
        attr_accessor :accept, :every, :inselection, :replacelabel, :replacebox
	...
	HISTORY_SIZE = 10
        ...
        def initialize(plugin)
          ...
          FXMAPFUNC(SEL_UPDATE,    ID_MODE_MATCHCASE,    :onUpdMatchCase)
          FXMAPFUNC(SEL_COMMAND,   ID_MODE_MATCHCASE,    :onCmdMatchCase)
          FXMAPFUNC(SEL_UPDATE,    ID_MODE_WHOLEWORD,    :onUpdWholeWord)
          FXMAPFUNC(SEL_COMMAND,   ID_MODE_WHOLEWORD,    :onCmdWholeWord)
          FXMAPFUNC(SEL_UPDATE,    ID_MODE_WRAP,         :onUpdWrap)
          FXMAPFUNC(SEL_COMMAND,   ID_MODE_WRAP,         :onCmdWrap)
          FXMAPFUNC(SEL_UPDATE,    ID_MODE_REGEXP,       :onUpdRegexp)
          FXMAPFUNC(SEL_COMMAND,   ID_MODE_REGEXP,       :onCmdRegexp)
          FXMAPFUNC(SEL_UPDATE,    ID_MODE_WORDSTART,    :onUpdWordStart)
          ...
          buttons = FXHorizontalFrame.new(self, LAYOUT_SIDE_BOTTOM|LAYOUT_FILL_X|PACK_UNIFORM_WIDTH|PACK_UNIFORM_HEIGHT)
          cancel = FXButton.new(buttons,"&Cancel",nil,self,ID_CANCEL,BUTTON_DEFAULT|FRAME_RAISED|FRAME_THICK|LAYOUT_FILL_Y|LAYOUT_RIGHT,0,0,0,0,HORZ_PAD,HORZ_PAD,VERT_PAD,VERT_PAD);
          ...
          @accept = accept
           pair = FXHorizontalFrame.new(buttons,LAYOUT_FILL_Y|LAYOUT_RIGHT, 0,0,0,0, 0,0,0,0);
          searchlast = FXArrowButton.new(pair,self,ID_PREV_MATCH,ARROW_LEFT|FRAME_RAISED|FRAME_THICK|LAYOUT_FILL_Y, 0,0,0,0,HORZ_PAD,HORZ_PAD,VERT_PAD,VERT_PAD);
          searchnext = FXArrowButton.new(pair,self,ID_NEXT_MATCH,ARROW_RIGHT|FRAME_RAISED|FRAME_THICK|LAYOUT_FILL_Y, 0,0,0,0,HORZ_PAD,HORZ_PAD,VERT_PAD,VERT_PAD);
          FXHorizontalSeparator.new(self,SEPARATOR_GROOVE|LAYOUT_SIDE_BOTTOM|LAYOUT_FILL_X);

          toppart = FXHorizontalFrame.new(self,LAYOUT_SIDE_BOTTOM|LAYOUT_FILL_X|LAYOUT_CENTER_Y, 0,0,0,0, 0,0,0,0, 10,10);
          FXLabel.new(toppart,nil,ic,ICON_BEFORE_TEXT|JUSTIFY_CENTER_X|JUSTIFY_CENTER_Y|LAYOUT_FILL_Y|LAYOUT_FILL_X)

          entry = FXVerticalFrame.new(toppart,LAYOUT_FILL_X|LAYOUT_CENTER_Y, 0,0,0,0, 0,0,0,0);
	  ...
	  replacebox.setNumVisible(5)
          ...
          FXCheckButton.new(options1,"Match whole word &only",self,ID_MODE_WHOLEWORD,ICON_BEFORE_TEXT|JUSTIFY_LEFT)
          FXCheckButton.new(options1,"&Wrap around",self,ID_MODE_WRAP,ICON_BEFORE_TEXT|JUSTIFY_LEFT)
          FXCheckButton.new(options1,"&Match case",self,ID_MODE_MATCHCASE,ICON_BEFORE_TEXT|JUSTIFY_LEFT)
          FXCheckButton.new(options1,"Regular e&xpression",self,ID_MODE_REGEXP,ICON_BEFORE_TEXT|JUSTIFY_LEFT)
          FXCheckButton.new(options1,"Match at word start",self,ID_MODE_WORDSTART,ICON_BEFORE_TEXT|JUSTIFY_LEFT)

          # Some hot keys Ctrl-f for forward search
          # Ctrl-b for backward search
          # Ctrl-r for find forward and replace 
	  ...
	  @searchbox.handle(self,MKUINT(0, SEL_KEYPRESS),fxevt)
        ...
        def searchMode=(mode)
          @searchmode = mode
          @plugin.properties['findreplace/search_mode'] = mode
        end
	  ...
	  @replace_history.slice!(-HISTORY_SIZE..-1) if @replace_history.size > HISTORY_SIZE
          ...
          self.searchMode = @plugin.properties['findreplace/search_mode'] || SEARCH_WRAP
	  ...
	  @plugin.properties.save if modified
	  ...
	  (self.searchMode & SEARCH_BACKWARD) != 0 ? onCmdPrev(sender,sel,ptr) : onCmdNext(sender,sel,ptr)
        ...
        def onUpdMatchCase(sender, sel, ptr)
          if (self.searchMode&SEARCH_MATCHCASE != 0)
        ...
        def onUpdWholeWord(sender, sel, ptr)
          if (self.searchMode&SEARCH_WHOLEWORD != 0)
            sender.handle(self,MKUINT(ID_CHECK,SEL_COMMAND),nil)
          else
            sender.handle(self,MKUINT(ID_UNCHECK,SEL_COMMAND),nil)
          end
        ...
        def onUpdWrap(sender, sel, ptr)
          if (self.searchMode&SEARCH_WRAP != 0)
            sender.handle(self,MKUINT(ID_CHECK,SEL_COMMAND),nil)
          else
            sender.handle(self,MKUINT(ID_UNCHECK,SEL_COMMAND),nil)
          end     
        ...
        def onUpdRegexp(sender, sel, ptr)
          if (self.searchMode&SEARCH_REGEXP != 0)
            sender.handle(self,MKUINT(ID_CHECK,SEL_COMMAND),nil)
          else
            sender.handle(self,MKUINT(ID_UNCHECK,SEL_COMMAND),nil)
          end     
        ...
        def onUpdBackslash(sender, sel, ptr)
          if (self.searchMode&SEARCH_BACKSLASH != 0)
            sender.handle(self,MKUINT(ID_CHECK,SEL_COMMAND),nil)
          else
            sender.handle(self,MKUINT(ID_UNCHECK,SEL_COMMAND),nil)
          end     
        ...
        def onCmdBackslash(sender, sel, ptr)
          self.searchMode ^= SEARCH_BACKSLASH
          return 1
        end

        def onUpdWordStart(sender, sel, ptr)
          if (self.searchMode&SEARCH_WORDSTART != 0)
            sender.handle(self,MKUINT(ID_CHECK,SEL_COMMAND),nil)
          else
            sender.handle(self,MKUINT(ID_UNCHECK,SEL_COMMAND),nil)
          end     
