...
#
# $Id: fox_debugger_configurator.rb,v 1.10 2005/10/20 14:46:08 jonathanm Exp $
#
...
module FreeRIDE
  module FoxRenderer
    ...
    @dbg_plugin.properties['default_interpreter'] = default_interpreter
    @dbg_plugin.properties['interpreters'].each do |key, val|
      if key == default_interpreter
        @dbg_plugin.properties['path_to_ruby'] = val["command"]
        break
      end
    end
    ...
    when 'Debugger'
      ...
      default_interpreter = @dbg_plugin.properties['default_interpreter']
  ...
  def cmp_prop(prop_name, val, default='')
    p = @dbg_plugin.properties[prop_name]
    return val==p if p
    return val==default
  end
