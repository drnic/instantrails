...
class File::Stat
   ...
   def initialize(file)
      ...
      @size  = stat_buf[24, 4].unpack('L').first # Size of file in bytes
      @atime = Time.at(stat_buf[32, 4].unpack('L').first) # Access time
      @mtime = Time.at(stat_buf[40, 4].unpack('L').first) # Modification time
      @ctime = Time.at(stat_buf[48, 4].unpack('L').first) # Creation time
      ...
      @blocks  = (@size.to_f / @blksize.to_f).ceil rescue nil # Fudge
